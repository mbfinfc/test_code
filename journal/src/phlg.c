...
main(int argc, char **argv)
{
    ...
    fd = open(argv[1], O_RDWR|O_CREAT|O_TRUNC|O_SYNC, 0664);
    if(fd < 0)
    {
        perror("open");
        return -errno;
    ...
    printf("writing %d bytes at location %llu in %s\n", sizeof(phobj_t), jHeader.jh_startOffset, argv[1]);
    ...
    ch1 = 'a', ch2 = 'A', ch3 = 'z';
    for(j = 0, i = 0; j < 1024; ++j)
    {
        sprintf(key,"/emFSPool0/emVfs0_%c%c%c_123/.admin%s",
            (ch1 > 'z' ? (ch1 = 'a') : ch1),
            (ch2 > 'Z' ? (ch2 = 'A') : ch2),
            (ch3 < 'a' ? (ch3 = 'z') : ch3),
            tmpnam(NULL));
        ...
        pPhobj = initPHObj(key);
        if(NULL == pPhobj)
        {
            fprintf(stderr,"Malloc failure\n");
            return -1;
        }
        pPhobj->NodeId = SrvPhsTableHashKey(key);
        printf("%d\n",pPhobj->NodeId);
        i += write(fd, pPhobj, pPhobj->size);
        free (pPhobj);
        pPhobj = NULL;
    }
    key[1025] = 'g';

    printf("Wrote %d bytes at location %llu in %s\n", i, jHeader.jh_startOffset, argv[1]);
    return 0;
}
